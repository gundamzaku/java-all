...
public class SerialMain {
	...
	public static void main(String[] args) {
		...
		List<BankMarketing> train = loader.load("NearestNeighbors\\data\\bank.data");
		System.out.println("Train: " + train.size());
		List<BankMarketing> test = loader.load("NearestNeighbors\\data\\bank.test");
		...
		try {
			Date start, end;
			start = new Date();
			for (BankMarketing example : test) {	//bank.test 2059次执行
